...
// 	acceptThread = m_acceptThread;
	...
	while (m_readThread)
	{
		...
		{
			TRACE(L"CloseHandle(m_readThread)\r\n");
	...
	while (m_acceptThread)
	{
			...
			watchDog >= THREAD_TERMINATION_MAXWAIT)
		{
			...
			TerminateThread(m_acceptThread, 0);//
			m_acceptThread = NULL;
		}
		watchDog++;
	}
...
bool CCESocket::Create(int socketType, int bufferSize)
{
	...
	switch(socketType)
	{
		case SOCK_STREAM:
			...
			//////////ADDED BY konyun
			if (setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (char*)&nNetTimeout, sizeof(int)) != 0)
			{
				m_errorCode = WSAGetLastError();
				closesocket(s);
				return FALSE;
			}
			if (setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (char*)&nNetTimeout, sizeof(int)) != 0)
			{
				m_errorCode = WSAGetLastError();
				closesocket(s);
				return FALSE;
...
void CCESocket::ReadThread()
{
	...
	//char * buf = NULL;//modify by konyun

	m_readThreadState = RUNNING;

	do  //
	{
		if (m_readThreadState == CLOSED)//added by konyun
		{
			TRACE(L"Port:%d,Close Readthread0\r\n", m_iRemotePort);
			break;
		}
		...
		m_bufRead = new char[bufSize];//buf = new char[bufSize];//
		//Blocking receive
		switch(m_socketType)
		{
			case SOCK_STREAM:
				//修改成发送BYTE 字节流，结构体。
				bytesRead = recv(s, m_bufRead, bufSize, 0);
				break;
			case SOCK_DGRAM:
				bytesRead = recvfrom(s, m_bufRead, bufSize, 0, (SOCKADDR*) &m_localAddress, &m_receiveAddrSz);
				break;
		}
		if (m_readThreadState == CLOSED)//added by konyun
		{
			TRACE(L"Port:%d,Close Readthread1\r\n", m_iRemotePort);
			break;
		}
		...
		if(bytesRead == SOCKET_ERROR || bytesRead <= 0)
		{
			...
			TRACE(L"Port:%d,Close Readthread2\r\n", m_iRemotePort);
			break;
		}
		if (m_readThreadState == CLOSED)//added by konyun
		{
			TRACE(L"Port:%d,Close Readthread3\r\n", m_iRemotePort);
			break;
		}
		...
		if(!OnReceive(m_bufRead, bytesRead))
		{
			data = new DataPacket;
			data->len = bytesRead;
			data->buf = m_bufRead; //data->buf = m_bufRead;//data->buf = buf;
			data->pos = m_bufRead; //data->pos = m_bufRead;//data->pos = buf;
	...
	TRACE(L"Port:%d,Close Readthread4\r\n", m_iRemotePort);
...
bool CCESocket::ReadString(CString &str)
{
	...
	/*	for(strPos=0; strPos < data->len; strPos++)
		...
		{*/
			...
			str = UTF8ToUnicode(buf);//str = buf; //Assign and convert to unicode
			if (buf)
			{
